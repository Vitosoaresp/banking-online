...
export function ComboBox({ selectedUserCashIn, setSelectedUserCashIn, users }: IComboboxProps) {
  const [queryPerson, setQueryPerson] = useState('');

  const userLoggedData = JSON.parse(localStorage.getItem('user') as string) as IUser;
      ...
      <div className='relative w-full bg-black text-zinc-300 rounded-md'>
        <div className='relative w-full cursor-default overflow-hidden rounded-md bg-black  text-left text-zinc-200 shadow-md focus:outline-none"'>
          <Combobox.Input
            ...
            displayValue={ (user: number) => users.find(u => u.id === user)?.username || '' }
            ...
            { filteredUser.map((user) => (
              ...
              <>
                ...
                {selected ? (
                  ...
                  >
                    <Check className="h-5 w-5" color="green" aria-hidden="true" />
                  </span>
                ) : null}
              </>
            )}
