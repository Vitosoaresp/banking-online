...
export function CreateTransferModal() {
  ...
  const userLoggedData = JSON.parse(localStorage.getItem('user') as string) as IUserLogin;
  ...
  async function handleSumbitTransfer(e: FormEvent) {
    ...
    if (balance === null || amount * 100 > balance) {
      ...
      try {
        ...
        console.log(error);
        setIsPromiseTransfer(false);
        alert('Não foi possível realizar a transferência, tente novamente mais tarde.');
      }
    }
  }
      ...
      <Dialog.Content className='fixed bg-zinc-900 py-16 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg md:w-[500px] w-[80vw] shadow-lg shadow-black/25'>
        <form onSubmit={handleSumbitTransfer} className='flex flex-col gap-5 md:px-8 w-full'>
          ...
          {errorTransferForBalance && (
            <p className='text-red-500'>Você não tem saldo suficiente para realizar essa transferência.</p>
          )}

          <div className='flex w-full gap-5 items-center justify-between'>
            <button
              disabled={selectedUserCashIn === '' || amount === 0.00 || isPromiseTransfer}
              ...
              type="submit"
            >
              {isPromiseTransfer ? <CircleNotch size={20} className='animate-spin' /> : 'Confirmar'}
            </button>
            <Dialog.Close className='text-zinc-200 font-medium bg-red-600 hover:bg-red-700 transition-colors py-4 px-6 text-lg rounded-md w-36'>Cancelar</Dialog.Close>
          </div>
        </form>
      </Dialog.Content>
    </Dialog.DialogPortal>
